...
- **It is the only implementation.** Every consumer goes through it: `runAnalysis`, `describeColumn`, `getColumnValues` ([driver.js](json/driver.js)), `collectDecodedColumns` and `buildCrosstab` ([exporters.js](json/exporters.js)). Do not re-derive the chain inline — the copies that used to exist had three different meta-fallback rules between them, which is exactly how the viewer and the analysis drifted apart.
- **Whose `meta` applies depends on whether the variant owns its payload:**
  - Base column, or a **pointer-style** variant (`col_vars[0]`, no `col_values` of its own, so it *is* the parent's payload) → the column's `meta`, merged per field with the variant's, so a variant declaring only `processing` still inherits the column's `replacements`.
  - A **derived** variant (carries its own `col_values`) → **only its own `meta`.** `variants.createVariant` builds it from the already-resolved source, so re-applying the column's replacements applies them twice. With a map whose range overlaps its domain (`a→b`, `b→c`) that visibly corrupts values, and `runAnalysis` did exactly this before the consolidation.
- `getColumnValues` keeps its "as stored" contract by delegating with `applyProcessing: false`; it is the one consumer that deliberately skips `meta`.
- The rule was independently arrived at by `variants.createVariant`, which used to pick its source with `(sourceVariant && sourceVariant.col_values) ? sourceVariant : baseCol` before it was folded into `resolveVariable`.
- **The write path resolves through it too.** `variants.createVariant` keeps its `(baseCol, config)` signature but wraps the column in a one-column database — `resolveVariable({columns:[{...baseCol, col_hash}]}, hash, config.sourceVarIndex)` — so a variant is built from exactly the view a reader will later see. It stores `meta.source_var_index` as the **resolved** index, never the requested one, because that field feeds snapshots' upstream-hash chain and must name the source actually used.
- One lookup stays inline on purpose: `updateColumnLabelsFromDb` ([driver.js](json/driver.js)) needs only `var_label`, so routing it through `resolveVariable` would decode every column's values on every label refresh.
...
- After `Plotly.newPlot`, `renderCharts` installs a per-div `ResizeObserver` that debounces `Plotly.Plots.resize(div)` on container-size changes. Fixes the well-known "chart renders at 700×450 fallback" when Bubble mounts placeholders before layout settles. Self-disconnects on detach; the outer MutationObserver also proactively disconnects on `removedNodes` to avoid leaks during reorders.
